...
def _fetch_single_dset(db_path: str, 
    ...
    """
    fetch name, mz, adduct, ccs, polarization, src_tag, smi, and chem_class_label for a dataset using
    ...
    Returns
    -------
    names, mzs, adducts, ccss, polarizations, srcs, smis, mqns, cls_labs : ``numpy.ndarray(...)``
    """
    con = connect(db_path)
    cur = con.cursor()
    qryA = 'SELECT g_id, name, mz, adduct, ccs, polarization, src_tag, smi, chem_class_label FROM master WHERE src_tag="{}"'.format(src_tag) + \
    ...
    names, mzs, adducts, ccss, polarizations, srcs, smis, cls_labs = [], [], [], [], [], [], [], []
    
    for _, name, mz, adduct, ccs, polarization, src, smi, cls_lab in cur.execute(qryA).fetchall():
        names.append(name)
        mzs.append(mz)
    ...
    return (array(names), array(mzs), array(adducts), array(ccs), array(polarizations),
            array(srcs), array(smis), array(mqns), array(cls_labs))
...
def _fetch_multi_dset(db_path: str, 
    ...
    """
    fetch name, mz, adduct, ccs, polarization, src_tag, smi, and chem_class_label for a set of datasets using
    the provided src_tag
    ...
    con = connect(db_path)
    cur = con.cursor()
    qryA = 'SELECT g_id, name, mz, adduct, ccs, polarization, src_tag, smi, chem_class_label FROM master WHERE src_tag IN ('
...
#                       encoded_adduct: bool = True, 
#                       mqn_indices: Optional[str | List[int]] = "all",
#                       use_polarization: bool = True  # New parameter to include polarization
#                       ) -> Any:
#         """
#         Assembles features for ML using a combination of m/z, encoded MS adduct (using 1-hot encoding), MQNs, 
#         and polarization (optional). Ensures all features are transformed into 1D arrays.

#         The final concatenated feature set is stored in the self.X_ instance variable.
...
#             Include encoded MS adduct in the feature set.
#         mqn_indices : str or List[int] or None, default="all"
#             Specify indices of MQNs to include in the feature set, or "all" to include all, or None to exclude.
#         use_polarization: bool, default=True
#             Include polarization feature in the feature set.
#         """
        
#         # 1D transformation for m/z
#         mz = self.mz_.flatten()  # Flatten to 1D array (N,)
        
#         # OneHot encode adducts (ensure 1D array)
#         ohe_adducts = None
#         if encoded_adduct:
#             self.OHEncoder_ = OneHotEncoder(sparse_output=False, categories='auto')
#             common_adducts = _filter_common_adducts(self.adduct_).reshape(-1, 1)
#             ohe_adducts = self.OHEncoder_.fit_transform(common_adducts).flatten()  # Flatten after encoding to 1D array
        
#         # Select MQNs and ensure they are 1D
#         use_mqns = None
#         if mqn_indices:
#             if self.mqn_.ndim == 1:
#                 self.mqn_ = self.mqn_.reshape(-1, 1)  # Convert 1D MQNs into 2D array (N, 1) if necessary
            
#             if mqn_indices == 'all':
#                 mqn_indices = [_ for _ in range(self.mqn_.shape[1])]  # Use all MQNs
            
#             # Select specified MQN indices and flatten to 1D array
#             use_mqns = self.mqn_[:, mqn_indices].flatten()  # Flatten to 1D array

#         # Handle polarization data and ensure it's 1D
#         use_polarization = None
#         if use_polarization and hasattr(self, 'polarization_'):
#             use_polarization = self.polarization_.flatten()  # Ensure polarization is 1D

#         # Concatenate all features (all are now 1D arrays)
#         features = [mz]
        
#         if ohe_adducts is not None:
#             features.append(ohe_adducts)
        
#         if use_mqns is not None:
#             features.append(use_mqns)

#         if use_polarization is not None:
#             features.append(use_polarization)
        
#         # Combine all 1D arrays into the final feature set
#         self.X_ = concatenate(features).reshape(-1, 1)  # Make sure the final array is 2D (N, 1) as required by ML models
#         self.n_features_ = self.X_.shape[1]  # Automatically adjusts with added features
#         self.y_ = self.ccs_  # Labels

#         # Featurize other datasets if applicable
#         if isinstance(self.datasets_, list):
#             for dset in self.datasets_:
#                 dset.assemble_features(encoded_adduct=encoded_adduct, 
#                                     mqn_indices=mqn_indices, 
...
#         Shuffles the data then splits it into a training set and a test set, storing each in self.X_train_,
#         self.y_train_, self.X_test_, self.y_test_ instance variables. The splitting is done in a stratified manner
#         based on either CCS or dataset source.
...
#             split for the train / test sets, respectively.
#         """
#         # Ensure features and labels have been generated
#         if self.X_ is None:
#             raise RuntimeError('C3SD: train_test_split: self.X_ is not initialized. Call assemble_features first.')
        
#         # Ensure stratify is valid
#         if stratify not in ['source', 'ccs']:
#             raise ValueError(f'C3SD: train_test_split: Invalid stratify="{stratify}". Must be "source" or "ccs".')
        
#         # Stratify based on source or CCS
#         if stratify == 'source':
#             y_cat = self.src_
#         else:
#             y_cat = self._get_categorical_y()  # Ensure this returns a valid 1D array
        
#         # Check if y_cat is a valid 1D array
#         if y_cat.ndim != 1:
#             raise ValueError('C3SD: train_test_split: y_cat must be a 1D array for stratification.')
...
#         """
#         if self.y_ is None:
#             raise ValueError("C3SD: _get_categorical_y: self.y_ is not initialized. Call assemble_features first.")
        
#         # Get quartiles from the label distribution (CCS values)
#         q1, q2, q3 = np.percentile(self.y_, [25, 50, 75])
        
#         # Compute midpoints for binning
#         mp12 = q1 + (q2 - q1) / 2.
#         mp23 = q2 + (q3 - q2) / 2.
        
#         # Define bin boundaries based on the computed quartiles and midpoints
#         bounds = [q1, mp12, q2, mp23, q3]
        
#         # Perform binning, returning a 1D array of categorical values
#         return np.digitize(self.y_, bounds)
